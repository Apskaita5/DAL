...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapEnum<T, TEnum> : ManagedOrmFieldMapBase<T, TEnum> 
        where T : class where TEnum : struct
    {
        ...
        public FieldMapEnum(string dbFieldName, string propName, Action<T, TEnum> valueSetter,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapEnum(string propName, Action<T, TEnum> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetEnum<TEnum>(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetEnum<TEnum>(PropName));
        }
