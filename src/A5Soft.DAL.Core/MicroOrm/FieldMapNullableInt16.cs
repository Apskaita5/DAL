...
{
    /// <summary>
    /// describes how a business object short? property (field) is persisted in a database;
    ...
    public sealed class FieldMapNullableInt16<T> : ManagedOrmFieldMapBase<T, short?> where T : class
    {
        ...
        public FieldMapNullableInt16(string dbFieldName, string propName, Action<T, short?> valueSetter,
            Func<T, short?> valueGetter, FieldPersistenceType persistenceType, int? updateScope = null)
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableInt16(string propName, Action<T, short?> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetInt16Nullable(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetInt16Nullable(PropName));
        }
