...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapNullableBool<T> : ManagedOrmFieldMapBase<T, bool?> where T : class
    {
        ...
        public FieldMapNullableBool(string dbFieldName, string propName, Action<T, bool?> valueSetter,
            Func<T, bool?> valueGetter, FieldPersistenceType persistenceType, int? updateScope = null)
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableBool(string propName, Action<T, bool?> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetBooleanNullable(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetBooleanNullable(PropName));
        }
