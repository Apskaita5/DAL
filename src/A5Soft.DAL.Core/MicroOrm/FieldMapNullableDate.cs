...
{
    /// <summary>
    /// describes how a business object Date? (DateTime without time) property (field) is persisted in a database;
    ...
    public sealed class FieldMapNullableDate<T> : ManagedOrmFieldMapBase<T, DateTime?> where T : class
    {
        ...
        public FieldMapNullableDate(string dbFieldName, string propName, Action<T, DateTime?> valueSetter,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableDate(string propName, Action<T, DateTime?> valueSetter, bool isInitializable = false)
        ...
        internal override SqlParam GetParam(T instance)
        {
            ...
            if (value.HasValue) value = value.Value.Date;
            return SqlParam.Create(DbFieldName, value);
        }

        internal override void SetValue(T instance, LightDataRow row)
        {
            var value = row.GetDateTimeNullable(PropName);
            if (value.HasValue) value = value.Value.Date;
        ...
        {
            var value = reader.GetDateTimeNullable(PropName);
            if (value.HasValue) value = value.Value.Date;
            ValueSetter(instance, value);
        }
