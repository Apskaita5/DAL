...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapDecimal<T> : ManagedOrmFieldMapBase<T, decimal> where T : class
    {
        ...
        public FieldMapDecimal(string dbFieldName, string propName, Action<T, decimal> valueSetter,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapDecimal(string propName, Action<T, decimal> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetDecimal(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetDecimal(PropName));
        }
