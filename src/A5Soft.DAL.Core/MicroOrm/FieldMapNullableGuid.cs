...
{
    /// <summary>
    /// describes how a business object Guid? property (field) is persisted in a database;
    ...
    public sealed class FieldMapNullableGuid<T> : ManagedOrmFieldMapBase<T, Guid?> where T : class
    {
        ...
        public FieldMapNullableGuid(string dbFieldName, string propName, Action<T, Guid?> valueSetter,
            Func<T, Guid?> valueGetter, FieldPersistenceType persistenceType, int? updateScope = null)
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableGuid(string propName, Action<T, Guid?> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetGuidNullable(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetGuidNullable(PropName));
        }
