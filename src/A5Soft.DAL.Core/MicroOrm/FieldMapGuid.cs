...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapGuid<T> : ManagedOrmFieldMapBase<T, Guid> where T : class
    {
        ...
        public FieldMapGuid(string dbFieldName, string propName, Action<T, Guid> valueSetter,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapGuid(string propName, Action<T, Guid> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetGuid(PropName));
        }
