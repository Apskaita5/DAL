...
{
    /// <summary>
    /// describes how a business object Enum (of type TEnum) property (field) is persisted in a database;
    ...
    /// <typeparam name="TEnum">a type of the enumeration</typeparam>
    public sealed class FieldMapNullableEnum<T, TEnum> : ManagedOrmFieldMapBase<T, TEnum?>
        where T : class where TEnum : struct
    {
        ...
        public FieldMapNullableEnum(string dbFieldName, string propName, Action<T, TEnum?> valueSetter,
            Func<T, TEnum?> valueGetter, FieldPersistenceType persistenceType, int? updateScope = null)
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableEnum(string propName, Action<T, TEnum?> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetEnumNullable<TEnum>(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetEnumNullable<TEnum>(PropName));
        }
