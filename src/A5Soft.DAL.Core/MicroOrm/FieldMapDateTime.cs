...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapDateTime<T> : ManagedOrmFieldMapBase<T, DateTime> where T : class
    {
        ...
        public FieldMapDateTime(string dbFieldName, string propName, Action<T, DateTime> valueSetter,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapDateTime(string propName, Action<T, DateTime> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetDateTime(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetDateTime(PropName));
        }
