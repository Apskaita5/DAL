...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapChar<T> : ManagedOrmFieldMapBase<T, char> where T : class
    {
        ...
        public FieldMapChar(string dbFieldName, string propName, Action<T, char> valueSetter,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapChar(string propName, Action<T, char> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetChar(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetChar(PropName));
        }
