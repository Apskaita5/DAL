...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapInt32<T> : ManagedOrmFieldMapBase<T, int> where T : class
    {
        ...
        public FieldMapInt32(string dbFieldName, string propName, Action<T, int> valueSetter,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapInt32(string propName, Action<T, int> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetInt32(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetInt32(PropName));
        }
