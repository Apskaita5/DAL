...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapDouble<T> : ManagedOrmFieldMapBase<T, double> where T : class
    {
        ...
        public FieldMapDouble(string dbFieldName, string propName, Action<T, double> valueSetter,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapDouble(string propName, Action<T, double> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetDouble(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetDouble(PropName));
        }
