...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapDate<T> : ManagedOrmFieldMapBase<T, DateTime> where T : class
    {
        ...
        public FieldMapDate(string dbFieldName, string propName, Action<T, DateTime> valueSetter,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapDate(string propName, Action<T, DateTime> valueSetter, bool isInitializable = false)
        ...
        {
            return SqlParam.Create(DbFieldName, ValueGetter(instance).Date);
        }

        internal override void SetValue(T instance, LightDataRow row)
        {
            ValueSetter(instance, row.GetDateTime(PropName).Date);
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetDateTime(PropName).Date);
        }
