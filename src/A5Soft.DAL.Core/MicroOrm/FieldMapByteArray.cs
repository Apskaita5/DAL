...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapByteArray<T> : ManagedOrmFieldMapBase<T, byte[]> where T : class
    {
        ...
        public FieldMapByteArray(string dbFieldName, string propName, Action<T, byte[]> valueSetter, 
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapByteArray(string propName, Action<T, byte[]> valueSetter,
            bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetByteArray(PropName));
        }
        
        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetByteArray(PropName));
        }
