...
{
    /// <summary>
    /// describes how a business object double? property (field) is persisted in a database;
    ...
    public sealed class FieldMapNullableDouble<T> : ManagedOrmFieldMapBase<T, double?> where T : class
    {
        ...
        public FieldMapNullableDouble(string dbFieldName, string propName, Action<T, double?> valueSetter,
            Func<T, double?> valueGetter, FieldPersistenceType persistenceType, int? updateScope = null)
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableDouble(string propName, Action<T, double?> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetDoubleNullable(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetDoubleNullable(PropName));
        }
