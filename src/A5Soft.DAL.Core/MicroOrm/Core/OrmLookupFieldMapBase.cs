...
{
    /// <summary>
    /// a base class for description of how a business object property (field) containing a lookup is persisted in a database;
    /// concrete implementations per field type should be added to a business type as a static field
    /// </summary>
    /// <typeparam name="TEntity">a type of the business object that the field belongs to</typeparam>
    public abstract class OrmLookupFieldMapBase<TEntity> where TEntity : class
    {
        /// <summary>
        ...
        protected OrmLookupFieldMapBase(string dbFieldName, string propName, FieldPersistenceType persistenceType,
            int? updateScope)
        {
        ...
        internal abstract SqlParam GetParam(TEntity instance);
        ...
        /// <param name="resolver">lookup resolver for fetching lookups</param>
        internal abstract Task SetValueAsync(TEntity instance, LightDataRow row, OrmServiceLookupResolver resolver);
        ...
        /// <param name="resolver">lookup resolver for fetching lookups</param>
        internal abstract Task SetValueAsync(TEntity instance, ILightDataReader reader, OrmServiceLookupResolver resolver);
        ...
        internal (string DbFieldName, string PropName) GetSelectField()
        {
            if (DbFieldName.IsNullOrWhiteSpace()) throw new InvalidOperationException(
                $"Database field name is not set for the property {PropName} in class {typeof(TEntity).FullName}.");
