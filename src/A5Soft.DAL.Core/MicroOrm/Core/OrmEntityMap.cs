...
namespace A5Soft.DAL.Core.MicroOrm.Core
{
    ...
    public sealed class OrmEntityMap<T> where T : class
    {
        ...
        private readonly List<OrmLookupFieldMapBase<T>> _lookupFields;
        ...
        public OrmEntityMap(Type mapType)
        {
            ...
            _lookupFields = staticFieldsInfo.Where(f => typeof(OrmLookupFieldMapBase<T>).IsAssignableFrom(f.FieldType))
                .Select(f => (OrmLookupFieldMapBase<T>)f.GetValue(null)).ToList();
        ...
        public (string DbFieldName, string PropName)[] GetFieldsForSelect()
        {
            ...
            result.AddRange(_lookupFields.Where(f => f.PersistenceType.HasFlag(FieldPersistenceType.Read))
                .Select(f => f.GetSelectField()));
        ...
        public string[] GetFieldsForInsert()
        {
            ...
            result.AddRange(_lookupFields.Where(f => f.PersistenceType.HasFlag(FieldPersistenceType.Insert)).
                Select(f => f.DbFieldName).ToList());
        ...
        public string[] GetFieldsForUpdate(int? scope)
        {
            ...
            result.AddRange(_lookupFields.Where(f =>
                f.PersistenceType.HasFlag(FieldPersistenceType.Update)
                && f.UpdateScope.IsInUpdateScope(scope, _identity.ScopeIsFlag))
                    .Select(f => f.DbFieldName));
        ...
        public SqlParam[] GetParamsForUpdate(T instance, int? scope)
        {
            ...
            result.AddRange(_lookupFields.Where(f => f.PersistenceType.HasFlag(FieldPersistenceType.Update)
                    && f.UpdateScope.IsInUpdateScope(scope, _identity.ScopeIsFlag))
                .Select(f => f.GetParam(instance)));
        ...
        /// </summary>
        public async Task<T> LoadInstanceAsync(LightDataRow row, OrmServiceLookupResolver lookupResolver)
        {
            var result = _identity.CreateInstance();
            await LoadValuesAsync(result, row, lookupResolver);
            return result;
        }
        ...
        public async Task LoadValuesAsync(T instance, LightDataRow row, OrmServiceLookupResolver lookupResolver)
        {
            ...
            foreach (var f in _lookupFields
                .Where(f => f.PersistenceType.HasFlag(FieldPersistenceType.Read)))
            {
                await f.SetValueAsync(instance, row, lookupResolver);
            }
        }
        ...
        /// Creates a new instance of business object and fills it with the data from database.
        /// </summary>
        public async Task<T> LoadInstanceAsync(ILightDataReader reader, OrmServiceLookupResolver lookupResolver)
        {
            var result = _identity.CreateInstance();
            await LoadValuesAsync(result, reader, lookupResolver);
            return result;
        }
        ...
        /// a query defined by <see cref="FetchQueryToken"/></param>
        public async Task LoadValuesAsync(T instance, ILightDataReader reader, OrmServiceLookupResolver lookupResolver)
        {
            if (!_identity.PrimaryKeyPropName.IsNullOrWhiteSpace())
            ...
            foreach (var f in _lookupFields
                .Where(f => f.PersistenceType.HasFlag(FieldPersistenceType.Read)))
            {
                await f.SetValueAsync(instance, reader, lookupResolver);
            }
        }
        ...
        /// </summary>
        public async Task<T> InitInstanceAsync(LightDataRow row, OrmServiceLookupResolver lookupResolver)
        {
            var result = _identity.CreateInstance();
            await InitValuesAsync(result, row, lookupResolver);
            return result;
        }
        ...
        public async Task InitValuesAsync(T instance, LightDataRow row, OrmServiceLookupResolver lookupResolver)
        {
            foreach (var f in _fields
        ...
        /// Creates a new instance of business object and initializes it with the data from database.
        /// </summary>
        public async Task<T> InitInstanceAsync(ILightDataReader reader, OrmServiceLookupResolver lookupResolver)
        {
            var result = _identity.CreateInstance();
            await InitValuesAsync(result, reader, lookupResolver);
            return result;
        }
        ...
        public async Task InitValuesAsync(T instance, ILightDataReader reader, OrmServiceLookupResolver lookupResolver)
        {
            foreach (var f in _fields
                .Where(f => f.PersistenceType.HasFlag(FieldPersistenceType.Init)))
            {
