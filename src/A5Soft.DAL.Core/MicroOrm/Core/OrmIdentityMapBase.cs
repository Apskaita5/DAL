...
namespace A5Soft.DAL.Core.MicroOrm.Core
{
    ...
    public abstract class OrmIdentityMapBase<T> where T : class
    {
            ...
            int? primaryKeyUpdateScope, bool scopeIsFlag, Func<T> createInstance)
        {
            ...
            PrimaryKeyFieldName = primaryKeyFieldName?.Trim() ?? String.Empty;
            PrimaryKeyPropName = primaryKeyPropName?.Trim() ?? String.Empty;
        ...
        internal abstract void LoadPrimaryKeyValue(T instance, ILightDataReader reader);
        ...
        {
            if (PrimaryKeyFieldName.IsNullOrWhiteSpace()) throw new InvalidOperationException(
                $"Primary key field name is not set for business class {typeof(T).FullName}.");
            if (PrimaryKeyPropName.IsNullOrWhiteSpace()) throw new InvalidOperationException(
                $"Primary key property name is not set for business class {typeof(T).FullName}.");
