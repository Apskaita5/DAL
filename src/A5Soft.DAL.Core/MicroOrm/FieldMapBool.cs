...
namespace A5Soft.DAL.Core.MicroOrm
{
    ...
    public sealed class FieldMapBool<T> : ManagedOrmFieldMapBase<T, bool> where T : class
    {
        ...
        public FieldMapBool(string dbFieldName, string propName, Action<T, bool> valueSetter,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapBool(string propName, Action<T, bool> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetBoolean(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetBoolean(PropName));
        }
