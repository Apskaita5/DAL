...
{
    /// <summary>
    /// describes how a business object char? property (field) is persisted in a database;
    ...
    public sealed class FieldMapNullableChar<T> : ManagedOrmFieldMapBase<T, char?> where T : class
    {
        ...
        public FieldMapNullableChar(string dbFieldName, string propName, Action<T, char?> valueSetter,
            Func<T, char?> valueGetter, FieldPersistenceType persistenceType, int? updateScope = null)
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableChar(string propName, Action<T, char?> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetCharNullable(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetCharNullable(PropName));
        }
