...
{
    /// <summary>
    /// describes how a business object byte? property (field) is persisted in a database;
    ...
    public sealed class FieldMapNullableByte<T> : ManagedOrmFieldMapBase<T, byte?> where T : class
    {
        ...
        public FieldMapNullableByte(string dbFieldName, string propName, Action<T, byte?> valueSetter,
            Func<T, byte?> valueGetter, FieldPersistenceType persistenceType, int? updateScope = null)
        ...
        public FieldMapNullableByte(string dbFieldName, string propName, Action<T, byte?> valueSetter,
            ...
            : base(dbFieldName, propName, isInitializable ?
                    FieldPersistenceType.Read | FieldPersistenceType.Init : FieldPersistenceType.Read,
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableByte(string propName, Action<T, byte?> valueSetter, bool isInitializable = false)
            : base(string.Empty, propName, isInitializable ?
                    FieldPersistenceType.Read | FieldPersistenceType.Init : FieldPersistenceType.Read,
                valueSetter, null, null) { }
        ...
        {
            ValueSetter(instance, row.GetByteNullable(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetByteNullable(PropName));
        }
