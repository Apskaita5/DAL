...
{
    /// <summary>
    /// describes how a business object int? property (field) is persisted in a database;
    ...
    public sealed class FieldMapNullableInt32<T> : ManagedOrmFieldMapBase<T, int?> where T : class
    {
        ...
        public FieldMapNullableInt32(string dbFieldName, string propName, Action<T, int?> valueSetter,
            Func<T, int?> valueGetter, FieldPersistenceType persistenceType, int? updateScope = null)
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableInt32(string propName, Action<T, int?> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetInt32Nullable(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetInt32Nullable(PropName));
        }
