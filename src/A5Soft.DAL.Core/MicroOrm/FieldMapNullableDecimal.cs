...
{
    /// <summary>
    /// describes how a business object decimal? property (field) is persisted in a database;
    ...
    public sealed class FieldMapNullableDecimal<T> : ManagedOrmFieldMapBase<T, decimal?> where T : class
    {
        ...
        public FieldMapNullableDecimal(string dbFieldName, string propName, Action<T, decimal?> valueSetter,
            Func<T, decimal?> valueGetter, FieldPersistenceType persistenceType, int? updateScope = null)
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableDecimal(string propName, Action<T, decimal?> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetDecimalNullable(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetDecimalNullable(PropName));
        }
