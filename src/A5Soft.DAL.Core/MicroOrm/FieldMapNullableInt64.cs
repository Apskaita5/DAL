...
{
    /// <summary>
    /// describes how a business object long? property (field) is persisted in a database;
    ...
    public sealed class FieldMapNullableInt64<T> : ManagedOrmFieldMapBase<T, long?> where T : class
    {
        ...
        public FieldMapNullableInt64(string dbFieldName, string propName, Action<T, long?> valueSetter,
            Func<T, long?> valueGetter, FieldPersistenceType persistenceType, int? updateScope = null)
        ...
        /// <param name="isInitializable">a value indicating whether the property value should be set (initialized) 
        /// from the init query result</param>
        public FieldMapNullableInt64(string propName, Action<T, long?> valueSetter, bool isInitializable = false)
        ...
        {
            ValueSetter(instance, row.GetInt64Nullable(PropName));
        }

        internal override void SetValue(T instance, ILightDataReader reader)
        {
            ValueSetter(instance, reader.GetInt64Nullable(PropName));
        }
