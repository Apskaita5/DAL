...
namespace A5Soft.DAL.Core
{
    ...
    public sealed class LightDataColumnCollection : ICollection<LightDataColumn>, IEnumerable<LightDataColumn>
    {
        ...
        private readonly List<LightDataColumn> _list;
        private readonly LightDataTable _dataTable;
        ...
        {
            var result = new LightDataColumn(GetDefaultColumnName());
            this.Add(result);
            return result;
        }
        ...
        {
            if (null == columns) throw new ArgumentNullException(nameof(columns));
            if (this.IsReadOnly)
                throw new InvalidOperationException(Properties.Resources.LightDataColumnCollection_CannotAddColumnToReadOnlyCollection);
        ...
        public bool Contains(string name)
        {
            ...
            return _list.Any(col => 
                col.ColumnName.Trim().Equals(name.Trim(), StringComparison.OrdinalIgnoreCase));
        }
        ...
        public int IndexOf(string name)
        {
            ...
            var col = _list.FirstOrDefault(c => 
                c.ColumnName.Trim().Equals(name, StringComparison.OrdinalIgnoreCase));

            if (col.IsNull()) return -1;
            return _list.IndexOf(col);
        }
        ...
        {
            return _list.Select(c => c.GetLightDataColumnProxy()).ToList();
        }
