...
namespace A5Soft.DAL.Core
{
    ...
    public sealed class LightDataRowCollection : ICollection<LightDataRow>, IEnumerable<LightDataRow>
    {
        ...
        private readonly List<LightDataRow> _list;
        ...
        {
            if (dataTable.IsNull()) throw new ArgumentNullException(nameof(dataTable));
        ...
        {
            if (row.IsNull()) throw new ArgumentNullException(nameof(row));
        ...
        internal async Task AddAsync(DbDataReader reader, CancellationToken ct)
        {
            do
            {
                _list.Add(new LightDataRow(_dataTable, reader));
        ...
        {
            return _list.Select(r => r.GetLightDataRowProxy()).ToList();
        }
